...
//     If only "other" was provided, the "You let yourself feel..." sentence
//     drops, but the profile gloss line still renders after the text quote.
//
...
// Map cant_shake_emotions (array of codes) → joined adjectives suitable for
// "You let yourself feel ___." Uses the participant's selected adjectives
// directly (no noun mapping; the prior angry→"anger" lookup was dropped).
// "other" is always stripped. Returns null when nothing usable remains so
// the caller can drop the entire "You let yourself feel..." sentence.
//
// Join form (intentionally simpler than joinProse's "a, and b" comma+and
// at 2, which fits long noun phrases for neighbors/contexts but reads
// stilted with bare adjectives):
//   1 item:  "angry"
//   2 items: "angry and grieved"          (no Oxford comma at 2)
//   3+ items: "angry, grieved, and weary" (Oxford comma at 3+)
const VALID_EMOTION_ADJECTIVES = [
  'angry','grieved','troubled','frightened','confused','ashamed','weary',
];
function humanizeEmotions(emotionCodes) {
  if (!Array.isArray(emotionCodes) || !emotionCodes.length) return null;
  const adjs = emotionCodes
    .filter((c) => c !== 'other')
    .filter((c) => VALID_EMOTION_ADJECTIVES.indexOf(c) >= 0);
  if (!adjs.length) return null;
  if (adjs.length === 1) return adjs[0];
  if (adjs.length === 2) return adjs[0] + ' and ' + adjs[1];
  return adjs.slice(0, -1).join(', ') + ', and ' + adjs[adjs.length - 1];
}
...
const STAGE3_PROFILES = {
  ...
  careful_seeker: {
    ...
    emotionGloss:  "You could have looked away. You didn't. That matters.",
  ...
  faithful_witness: {
    ...
    emotionGloss:  "That this still moves you, after everything, is grace.",
...
function renderStage3Email(opts) {
  ...
  //
  // New v1.1 structure (replaces the awkward "The feeling that came with
  // it: an anger, a grief." noun-mapped form):
  //
  //   "In the discernment, you named what you can't shake: '[text]'.
  //    You let yourself feel [adjectives]. [Profile gloss]."
  //
  // - If cantShakeEmotions is empty (or contains only "other"), the
  //   "You let yourself feel..." sentence drops and the paragraph reads
  //   text + gloss directly.
  // - Profile gloss line ALWAYS appears when cantShakeText is present
  //   (regardless of whether the emotion sentence renders). Verified to
  //   read cleanly in both states for all five profiles.
  if (cantShakeText) {
    let paragraph = ''
      ...
      + '.';
    if (emotionPhrase) {
      paragraph += ' You let yourself feel ' + esc(emotionPhrase) + '.';
    }
